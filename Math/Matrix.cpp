...
template<typename T>
Mat4<T> &Mat4<T>::translate(const Vec3<T> &v) {
...
template<typename T>
Mat4<T> &Mat4<T>::rotate(T angle, T x, T y, T z) {
...
template<typename T>
Mat4<T> &Mat4<T>::scale(const Vec3<T> &v) {
    return scale(v.x, v.y, v.z);
}

template<typename T>
Mat4<T>& Mat4<T>::multiply(const Mat4<T>& matrix) {
...
template<typename T>
Mat4<T>& Mat4<T>::multiply(const Mat4<T>& one, const Mat4<T>& two) {
...
template<typename T>
Mat4<T> &Mat4<T>::removeTranslation() {
    m30 = 0.0;
    m31 = 0.0;
    m32 = 0.0;
    return *this;
...
template<typename T>
Mat4<T> &Mat4<T>::perspective(T fov, T width, T height, T near, T far){
...
template<typename T>
const T* Mat4<T>::getBuffer() const {
    return &m00;
}
